...
async function injectListingBadges() {
  ...
  if (DEBUG_CONFIG.enabled) {
    console.log(`[CANADA-LIST] Processing ${containers.length} product containers`);
  }
  ...
  if (DEBUG_CONFIG.enabled && DEBUG_CONFIG.logPerformance) {
    console.log(`[CANADA-LIST] Completed processing ${containers.length} containers in ${timing.duration.toFixed(2)}ms`);
  }
}
...
const observer = new MutationObserver(() => {
  ...
  observerTimer = setTimeout(() => {
    if (DEBUG_CONFIG.enabled) {
      console.log('[CANADA-LIST] DOM mutation detected, re-scanning for new products');
    }
    init();
  }, 350);
});
...
// Expose performance tracker for debugging
if (DEBUG_CONFIG.enabled) {
